...
4. **Build commands:** This project has no build step and no `package.json`. In **Settings → General**, set **Build Command** and **Install Command** to *empty* (leave them blank, or override with a single space). That way Vercel won’t run `npm install` / `npm run build` and will just deploy the `api/` function.
...
9. **If you get 404 NOT_FOUND:** Vercel is not seeing your `api/` folder. In the Vercel project go to **Settings → General → Root Directory**: set it to the folder that **contains** the `api` folder (e.g. `playground-serveless` if this code lives in a subfolder of your repo). Save and **redeploy**.
